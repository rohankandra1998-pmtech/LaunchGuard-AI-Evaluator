...
export type PreparedGeneratedSuggestion = GeneratedQuestionSuggestion & {
...
export function initialSuggestionVariableValues(schema: PromptVariable[]): Record<string, TestCaseVariableValue> {
  return Object.fromEntries(schema.map((variable) => [variable.key, fallbackValue(variable)]));
}
