      ...
      >
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
          <Card>
            ...
            <label className="mt-5 block text-sm font-medium text-guard-text">System Prompt<TextArea ref={promptRef} required name="system_prompt" value={systemPrompt} onChange={(event) => setSystemPrompt(event.target.value)} className="mt-2 min-h-96 resize-y bg-guard-surfaceMuted font-mono leading-6" placeholder="Write the system prompt for this version..." /></label>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card><h2 className="text-lg font-semibold text-guard-ink">Final Prompt Preview</h2><p className="mt-1 text-sm text-guard-muted">The exact compiled system prompt sent to the model.</p>{preview.errors.length ? <div className="mt-4 rounded-lg border border-amber-200 bg-guard-amberSoft p-3 text-sm text-guard-amber">{preview.errors.join(" ")}</div> : null}<pre className="mt-4 min-h-64 whitespace-pre-wrap break-words rounded-lg border border-guard-line bg-guard-surfaceMuted p-4 text-sm leading-6 text-guard-text">{preview.compiledPrompt || "Your compiled prompt will appear here."}</pre></Card>
