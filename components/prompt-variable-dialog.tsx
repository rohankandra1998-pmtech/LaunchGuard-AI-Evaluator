...
export function PromptVariableDialog({
  ...
  return (
    <dialog
      ...
      className="m-auto max-h-[calc(100vh-2rem)] w-[calc(100%-2rem)] max-w-2xl overflow-y-auto rounded-2xl border border-guard-line bg-white p-0 text-left text-guard-text shadow-floating backdrop:bg-slate-900/35 backdrop:backdrop-blur-sm"
    >
      <div className="p-5 sm:p-6">
        {confirmingRemove ? (
          <div>
            ...
            <p id={descriptionId} className="mt-2 text-sm leading-6 text-guard-muted">The variable configuration will be removed. Any remaining placeholder will become unresolved and block testing or saving.</p>
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setConfirmingRemove(false)} className="focus-ring rounded-lg border border-guard-lineStrong bg-white px-4 py-2 text-sm font-semibold text-guard-text hover:bg-guard-surfaceMuted">Keep variable</button>
          ...
          <>
            ...
            <p id={descriptionId} className="mt-2 text-sm leading-6 text-guard-muted">Configure a version-specific value that can be inserted as <code className="text-guard-primary">{"{{variable_name}}"}</code>.</p>
            <div className="mt-6 grid gap-5">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="text-sm font-medium text-guard-text">
                  Variable name
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="flex items-center gap-3 self-start pt-8 text-sm font-medium text-guard-text"><input type="checkbox" checked={draft.required} onChange={(event) => setDraft({ ...draft, required: event.target.checked })} className="h-4 w-4 accent-guard-primary" />Required value</label>
              </div>
              {draft.type === "select" ? (
                <label className="text-sm font-medium text-guard-text">Options, one per line<TextArea value={options} onChange={(event) => setOptions(event.target.value)} className="mt-2 min-h-28" aria-invalid={Boolean(errors.options)} />{errors.options ? <span className="mt-1 block text-xs text-guard-red">{errors.options}</span> : null}</label>
              ) : null}
              <label className="text-sm font-medium text-guard-text">
                Default value
              ...
              <label className="text-sm font-medium text-guard-text">Description / help text<TextArea value={draft.description || ""} onChange={(event) => setDraft({ ...draft, description: event.target.value })} className="mt-2 min-h-20" />{errors.description ? <span className="mt-1 block text-xs text-guard-red">{errors.description}</span> : null}</label>
            </div>
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row"><button type="button" onClick={close} className="focus-ring rounded-lg border border-guard-lineStrong bg-white px-4 py-2 text-sm font-semibold text-guard-text hover:bg-guard-surfaceMuted">Cancel</button><button type="button" onClick={save} className="focus-ring rounded-lg bg-guard-primary px-4 py-2 text-sm font-semibold text-white hover:bg-guard-primaryHover">Save Variable</button></div>
            </div>
          </>
        )}
      </div>
