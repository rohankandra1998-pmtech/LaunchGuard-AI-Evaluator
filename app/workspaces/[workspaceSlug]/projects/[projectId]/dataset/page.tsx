...
export default async function DatasetPage({ params }: { params: Promise<{ workspaceSlug: string; projectId: string }> }) {
  ...
  return <div><PageHeader eyebrow="Golden dataset" title="Golden Dataset">Create, filter, and run test cases against selected prompt versions. Outputs are saved with the prompt version and model used.</PageHeader><DatasetWorkspace workspaceSlug={workspaceSlug} project={project} promptVersions={prompts || []} criteria={criteria || []} testCases={testCases || []} /></div>;
}
